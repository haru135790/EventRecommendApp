...
Deno.serve(async (req) => {
    ...
    if (req.method === "POST" && pathname === "/add-event") {
        ...
        return new Response(JSON.stringify({ message: "イベント追加成功" }), {
            status: 200,
        });
    }
    ...
    if (req.method === "POST" && pathname === "/delete-event") {
        ...
        if (!event.value) {
            return new Response(
                JSON.stringify({ message: "イベントが見つかりません" }),
                { status: 404 },
            );
        }

        await kv.delete(["event", username, id]);
        return new Response(JSON.stringify({ message: "イベント削除成功" }), {
            status: 200,
        });
    }
    ...
    if (req.method === "POST" && pathname === "/api/register") {
        ...
        if (!username || !password) {
            return new Response(
                JSON.stringify({ message: "ユーザー名とパスワードは必須です" }),
                { status: 400 },
            );
        }
        // 既存ユーザー確認
        ...
        return new Response(JSON.stringify({ message: "ユーザー登録成功" }), {
            status: 201,
        });
    }
    ...
    if (req.method === "POST" && pathname === "/api/login") {
        ...
        return new Response(
            JSON.stringify({ message: "ログイン成功", id: user.id }),
            { status: 200 },
        );
    }
